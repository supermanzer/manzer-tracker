...
fun WorkoutSessionItem(session: WorkoutSession, plan: WorkoutPlan?, onClick: () -> Unit) {
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            ...
            session.endTime?.let {
                val durationMs = it.time - session.startTime.time
...
fun WorkoutSessionDetail(
    ...
    Column(
        ...
        DetailRow(
            label = "Start Time",
            value = SimpleDateFormat("MMM dd, yyyy HH:mm", Locale.getDefault()).format(session.startTime)
        )
        session.endTime?.let {
            DetailRow(
                label = "End Time",
                value = SimpleDateFormat("MMM dd, yyyy HH:mm", Locale.getDefault()).format(it)
            )
        }
